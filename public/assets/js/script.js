...
let search = function (input) {
    ...
    fetch(
        'https://api.themoviedb.org/3/search/multi?api_key=14b7c2e67f36427d72ce8c1df6482552&query=' +
            input.toLowerCase()
    )
        .then(function (res) {
            if (res.ok) {
                ...
                for (let i = 0; i < x; i++) {
                    ...
                    if (current.title) {
                        ...
                        if (current.first_air_date) {
                            showYear.innerText =
                                current.first_air_date.substring(0, 4); //sometimes set to none
                        } else {
                            showYear.innerText = 'N/A ';
                        }
                    }
...
let suggestionSelect = function (evt) {
    let current = evt.target;
    if (current.id !== 'suggestion-container') {
        //Ensure a suggestion is clicked on
...
function watchProviders(showType, showID, showYear) {
    ...
    fetch(
        'https://api.themoviedb.org/3/' +
            ...
            '/' +
            showID +
            '?api_key=14b7c2e67f36427d72ce8c1df6482552'
    )
        .then(function (res) {
            if (res.ok) {
                return res.json();
            ...
            try {
                ...
                fetch(
                    'https://api.themoviedb.org/3/' +
                        ...
                        '/' +
                        showID +
                        '/watch/providers?api_key=14b7c2e67f36427d72ce8c1df6482552'
                )
                    .then(function (res) {
                        if (res.ok) {
                            return res.json();
                    ...
                    .then(function (data) {
                        try {
                            ...
                            // viewer rating
                            selectedScoreEl.innerText =
                                'Viewer Rating: ' +
                                selectedTitleData.vote_average +
                                '/10';

                            // poster
                            if (selectedTitleData.poster_path) {
                                selectedPosterEl.src =
                                    'https://image.tmdb.org/t/p/w500' +
                                    selectedTitleData.poster_path;
                            } else {
                                selectedPosterEl.src =
                                    'https://via.placeholder.com/500x750.png?text=Movie+Poster';
                            }
                            ...
                            // plot
                            selectedPlotEl.innerText =
                                selectedTitleData.overview;
                            ...
                            for (let i = 0; i < providerData.length; i++) {
                                if (providerData.length >= 1) {
                                    ...
                                    providerLogo.src =
                                        ...
                                        providerData[i].logo_path;
                                    providerLogo.alt =
                                        providerData[i].provider_name;
                            ...
                            noProvidersP.innerText =
                                'Sorry, there are no watch providers for this title.';
...
function themeRemover() {
    document
        .querySelector('a')
        .classList.remove('tyler', 'solo-jazz', 'darkmode');
    document
        .querySelector('#form-submit')
        .classList.remove('tyler', 'solo-jazz', 'darkmode');
    document
        ...
        .classList.remove('tyler', 'solo-jazz', 'darkmode');
    document
        .querySelector('#queue-button')
        .classList.remove('tyler', 'solo-jazz', 'darkmode');
    document
        .querySelector('#delete-all-queue')
        .classList.remove('tyler', 'solo-jazz', 'darkmode');
    document
        .querySelector('#delete-all-queue2')
        .classList.remove('tyler', 'solo-jazz', 'darkmode');
    var deleteButtonEls = document.getElementsByClassName('delete-btn');
    for (let i = 0; i < deleteButtonEls.length; i++) {
    ...
    document
        ...
        .classList.remove('tyler', 'solo-jazz', 'darkmode');
    document
        .querySelector('#logo-image-2')
        .classList.remove('tyler', 'solo-jazz', 'darkmode');
    document
        .querySelector('#user-input')
        .classList.remove('tyler', 'solo-jazz', 'darkmode');
    document
        .querySelector('#user-input2')
        .classList.remove('tyler', 'solo-jazz', 'darkmode');
    document
        .querySelector('#queue-header-1')
        .classList.remove('tyler', 'solo-jazz', 'darkmode');
    document
        .querySelector('#queue-header-2')
        .classList.remove('tyler', 'solo-jazz', 'darkmode');
    document
        .querySelector('#search-queue')
        .classList.remove('tyler', 'solo-jazz', 'darkmode');
    document
        .querySelector('#search-queue2')
        .classList.remove('tyler', 'solo-jazz', 'darkmode');
    document
        .querySelector('#suggestions-header')
        .classList.remove('tyler', 'solo-jazz', 'darkmode');
    document
    ...
    document
        ...
        .classList.remove('tyler', 'solo-jazz', 'darkmode');
    document
        .querySelector('#selected-score')
        .classList.remove('tyler', 'solo-jazz', 'darkmode');
    document
        .querySelector('#selected-plot')
        .classList.remove('tyler', 'solo-jazz', 'darkmode');
    document
        .querySelector('#streamability-title')
        .classList.remove('tyler', 'solo-jazz', 'darkmode');
    document
